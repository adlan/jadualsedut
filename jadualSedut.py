...
class CourseClass:
    ...
    def view(self):
        return '[%s] %8s - %-8s %-10s %s' % (
                self.group, self.timeStart, self.timeEnd, self.day, self.room)

    def zenity_list(self):
        return ['FALSE', self.group, self.timeStart, self.timeEnd, self.day, self.room]
...
def gui(data, course):
    string = reduce(lambda x,y: x+y, [i.zenity_list() for i in data])
    zenity = ['zenity', '--list', '--width=500', '--height=250',
              '--title="Jadual"', '--text=%s' % (course,), '--checklist']
    columns = [' ', 'Kumpulan', 'Mula', 'Tamat', 'Hari', 'Bilik']
    zenity.extend(['--column=%s' % (col,) for col in columns])
    zenity.extend(string)
    timeTable = Popen(zenity, stdin=PIPE, stdout=PIPE).communicate()[0].strip("\n")
...
def main():
    ...
    if faculty is None or course is None:
        usage()
        return 1
